...
object PauseMenu:
  def apply(): Parent =
    lazy val backgroundImage = createBackgroundView("/pause_menu.jpg", PAUSE_MENU_SCALE_FACTOR).getOrElse(new ImageView())
    lazy val titleImage =
      createImageView("/paused.png", (backgroundImage.fitWidth*PAUSE_TITLE_SCALE_FACTOR).toInt) match
